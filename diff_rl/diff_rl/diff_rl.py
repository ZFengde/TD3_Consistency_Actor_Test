    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                next_actions = (self.consistency_model.sample(model=self.actor, state=replay_data.next_observations) + noise).clamp(-1, 1)
            ...
            if self._n_updates % self.policy_delay == 0:
                ...
                with th.set_grad_enabled(True):
                    action_temp = replay_data.actions.clone().detach().requires_grad_(True)
                    q_value = self.critic.q1_forward(replay_data.observations, action_temp).mean()
                    q_value.backward()
                    dq_a = action_temp.grad # target_actions = replay_data.actions - 0.05 * dq_a, which will make actions better
                    normalised_dq_a = (dq_a - dq_a.mean(dim=1, keepdim=True)) / dq_a.std(dim=1, keepdim=True)
