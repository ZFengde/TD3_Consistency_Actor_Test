...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        self.consistency_model = Consistency_Model(action_dim=action_dim, 
                                                    sigma_max=80.0,
                                                    device=self.device,
                                                    sampler="onestep",)
    ...
    def _sample_action(
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            ...
            unscaled_action = self.consistency_model.sample(model=self.actor, state=state_rpt)
            q_value = self.critic_target.q1_forward(state_rpt, unscaled_action).flatten()
            
            idx = th.multinomial(F.softmax(q_value, dim=0), num_samples=1)
            unscaled_action = unscaled_action[idx].cpu().data.numpy()
